...
module.exports = (config = null, callback = null) => {
  if (!config)
    throw new Error("missing config object in db delete function. Must provide a config object as first parameter");
  ...
  Models[active_options.from].deleteOne(active_options.query, (err) => {
    if (err) {
      console.trace({ err })
    }

    if(!callback || (typeof callback !== "function"))
      throw new Error("Missing callback function in db delete function. callback must be a function")
